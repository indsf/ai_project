...
def _resolve_data_path(filename: str) -> Path:
    """
    PLACES_DATA_DIR을 우선 확인하고, 없으면 팀원들이 흔히 쓰는 대체 위치들을
    순서대로 찾아본다. .env에 PLACES_DATA_DIR을 안 맞춰도 웬만하면 알아서 찾도록.
    """
    project_root = Path(__file__).resolve().parent.parent.parent.parent

    candidates = [
        PLACES_DATA_DIR / filename,
        project_root / "data" / "tourapi" / filename,
    ...
    # 못 찾았으면 원래 에러 메시지에 시도한 경로들을 다 보여준다.
...
def seed_category(
    ...
    target_path = path or _resolve_data_path(meta["file"])
...
class PlaceSummary:
    ...
    def __init__(self, place: Place):
        ...
        self.distance_km = getattr(place, "distance_km", None)
